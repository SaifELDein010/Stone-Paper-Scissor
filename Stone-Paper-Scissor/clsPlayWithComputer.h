...
private:
	...
	static void _GetPlayersChoise() {
		...
		Player2.PalyerChoise = clsGameCore::ComputerChoise();
	...
	static string _RoundWinnerText(short whoWin) {
		...
		string arr[3] = { "Draw", "Player", "Computer" };
	...
	static void _ShowRoundScreen(short roundNumber) {
		...
		cout << "\nPlayer choise  : " << Player1.GetPlayerChoiseText();
		cout << "\nComputer choise: " << Player2.GetPlayerChoiseText();
	...
	static void _FinalBoard() {
		...
		cout << clsInput::Format(5, "\t") << clsInput::Format(3, " ") << "Player win times   : " << Player1.GetPlayerWinTime() << '\n';
		cout << clsInput::Format(5, "\t") << clsInput::Format(3, " ") << "Computer win times : " << Player2.GetPlayerWinTime() << '\n';
